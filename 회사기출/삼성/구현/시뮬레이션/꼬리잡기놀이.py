...
def dfs(x, y, idx):
    ...
    for dx, dy in zip(dxs, dys):
        ...
        if is_out_range(nx, ny):
            continue
        # 만약 빈칸이라면(머리사람, 나머지사람, 꼬리사람, 이동선 모두 아니라면)
        if board[nx][ny] == 0:
            continue
        # 만약 이미 방문했다면
        if visited[nx][ny]:
...
def init():
    ...
    # 각 팀별로 머리사람 좌표를 반복하면서, dfs를 통해 팀별로 레일 좌표를 리스트에 순서대로 넣기
...
def move_all():
    ...
    # 팀 개수만큼 반복하면서
    for i in range(1, m + 1):
        # 각 팀에 대해 레일을 한 칸씩 앞으로(머리사람-> 이동선 방향) 이동(머리사람만 빼고)
        # 현재팀의 레일 중 가장 마지막 레일
        tmp = v[i][-1]
        # 레일 좌표를 뒤에서부터 반복하면서
        for j in range(len(v[i]) - 1, 0, -1):
            # 레일을 한칸씩 앞으로 이동
            v[i][j] = v[i][j - 1]
        # 머리사람 좌표를 리스트의 맨 앞으로 이동(이동하기 이전의 마지막 레일 좌표가 현재 머리사람 좌표가 되므로)
        v[i][0] = tmp
...
# 공을 획득한 팀의 순서를 바꾸기(머리사람 -> 꼬리사람, 꼬리사람 -> 머리사람)
