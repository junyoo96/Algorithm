# 9:30~ - 아이디어 생각 못함
# 10:33~11:02 / 11:57~ - 경우의 수는 맞았지만 영역합 구하는 부분 구현 못함
# 4:30~5:00/5:00~ - 아이디어와 구현 모두 생각했지만 구현에서 오류나서 틀림
...
for _ in range(n):
    ...
    data.append([0] + list(map(int, input()))) # 주의 - 붙어있는 숫자 따로 받는 방법

# 합을 저장할 리스트
sum_arr = [[0 for _ in range(m + 1)] for _ in range(n + 1)]

# 중요, 주의 - 직사각형의 1,1부터 현재 좌표 영역 내 모든 수의 합을 미리 계산해서 해당 좌표 칸에 저장
for i in range(1, n + 1):
    for j in range(1, m + 1):
        # 주의 - 빼야되는 부분 좌표 주의
        sum_arr[i][j] = sum_arr[i - 1][j] + sum_arr[i][j - 1] - sum_arr[i - 1][j - 1] + data[i][j]

# 중요, 주의 - 주어진 영역에 속한 수의 합을 계산하는 함수
