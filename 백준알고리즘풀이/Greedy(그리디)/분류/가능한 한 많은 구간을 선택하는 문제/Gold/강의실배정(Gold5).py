...
# 9:29~10:08 / 10:08~ - 틀림(heappop 사용해 강의실 수업 제거하는 알고리즘 구현 못함)
...
for i in range(1, n):
    # 만약 현재 수업의 시작시간이 강의실 수업의 끝 시간보다 같거나 이후이면
        # 주의 - 첫번째 강의실의 끝시간만 보는 이유는 만약 현재 수업 시작시간이 첫번째 강의실 수업시간의 끝보다 이전일 경우,
        # 두번째 강의실 이후부터는 첫번째 강의실 수업 끝시간보다 끝시간이 더 이후므로 현재 수업이 들어갈 수 없기 때문
